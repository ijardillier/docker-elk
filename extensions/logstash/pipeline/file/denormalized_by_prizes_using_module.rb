require './script/denormalized_by_prizes_helper.rb'
#require_relative '../../script/denormalized_by_prizes_helper.rb'
...
def filter(event)
    ...
    originalEvent = LogStash::Util::DenormalizationByPricesHelper::getOriginalEvent(event, @keep_original_event);
    if not originalEvent.nil?
        items.push originalEvent
    end

    # Get prizes items (to denormalize)
    prizes = LogStash::Util::DenormalizationByPricesHelper::getPrizes(event);
    if prizes.nil?
        return items
    end
   
    # Create a clone base event
    eventBase = LogStash::Util::DenormalizationByPricesHelper::getEventBase(event);

    # Create one event by prize item with needed modification
    prizes.each { |prize| 
        items.push LogStash::Util::DenormalizationByPricesHelper::createEventForPrize(eventBase, prize);
    }
